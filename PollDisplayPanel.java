...
public class PollDisplayPanel extends JPanel
{
  ...
  private void drawPieChart(Graphics g, int x, int y, int r)
  {
    ...
    if (total > 0)
    {
      ...
      drawSector(g, x, y, r, fromDegree, degrees); //Draw sector from 0 to degrees
      fromDegree = fromDegree + degrees;
      
      g.setColor(Color.GREEN);
      degrees = countToDegrees(count2, total);
      drawSector(g, x, y, r, fromDegree, degrees); //Draw sector from Red's degrees to (Red's degrees + Green's degrees)
      fromDegree = fromDegree + degrees;
      
      
      g.setColor(Color.BLUE);
      degrees = Math.max(360-fromDegree, 0); //Set degrees to the higher of:  The remaining degrees and Zero
      drawSector(g, x, y, r, fromDegree, degrees); //Draw from Green's degrees to 360.
      
//_______

    }
    else //If total is zero make things light grey
    {
